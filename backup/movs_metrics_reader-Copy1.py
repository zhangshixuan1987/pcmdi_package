...
class MoVsMetricsReader:
    ...
    def _read_metrics(self):
        ...
        if self.ref_name is not None:
            ...
            if not ref_files:
                raise FileNotFoundError("ERROR: ref_name provided but no reference MoVs metric files found.")
            missing = [f for f in ref_files if not os.path.exists(f)]
            if missing:
                raise FileNotFoundError("ERROR: Missing reference MoVs metric files: " + ", ".join(missing))
            logger.info("Found reference MoVs metrics, reading…")
            ref_lib = self._load_movs_files(ref_files)  # {mode -> RESULTS(runs_dict)}

        # --- collect all test models ---
        model_lib = {}  # {mode -> {model_name -> runs_dict}}
        for test_mip, model_name, case_id in zip(self.mips, self.tests, self.caseids):
            ...
            for mode in self.movs_mode:
                ...
                if model_files:
                    ...
                    logger.debug(f"[variability_modes] No files for mode={mode}, obs={obs_str}")
            ...
            raw = self._load_movs_files(all_model_files)  # {mode -> RESULTS(runs_dict)}

            for mode, runs_dict in (raw or {}).items():
                if not runs_dict:
                    continue
                model_lib.setdefault(mode, {})
                if model_name in model_lib[mode]:
                    raise ValueError(f"Duplicate data for mode={mode}, model={model_name}")
                model_lib[mode][model_name] = runs_dict  # <-- runs_dict: {run_id -> ...}
    ...
    def collect_metrics(self):
        ref_lib, model_lib = self._read_metrics()

        cleaned_ref_lib = None
        if not self.test_model_only and ref_lib:
            cleaned_ref_lib = self._prune_empty_dfs(ref_lib)

        cleaned_model_lib = self._prune_empty_dfs(model_lib)

        merged_lib, ref_df_lib, test_df_lib = {}, {}, {}
        mode_season_list = []

        if cleaned_ref_lib is not None:
            # with reference
            for stat, _ in self.diag_vars.items():
                ref_df, mode_season_ref = self._movs_dict_to_df(cleaned_ref_lib, self.group, stat)
                test_df, mode_season_tst = self._movs_dict_to_df(cleaned_model_lib, self.group, stat)

                # track common mode-season cols
                for m in mode_season_ref:
                    if m in mode_season_tst and m not in mode_season_list:
                        mode_season_list.append(m)

                # align columns and make 'model' first
                common_cols = sorted(set(ref_df.columns) | set(test_df.columns))
                ref_df = ref_df.reindex(columns=common_cols)
                test_df = test_df.reindex(columns=common_cols)
                ref_df = ref_df.reindex(columns=["model"] + [c for c in ref_df.columns if c != "model"])
                test_df = test_df.reindex(columns=["model"] + [c for c in test_df.columns if c != "model"])

                ref_df_lib[stat] = ref_df
                test_df_lib[stat] = test_df
                merged = pd.concat([ref_df, test_df], ignore_index=True)
                merged_lib[stat] = merged.reindex(columns=["model"] + [c for c in merged.columns if c != "model"])
        else:
            # tests only
            for stat, _ in self.diag_vars.items():
                test_df, mode_season_tst = self._movs_dict_to_df(cleaned_model_lib, self.group, stat)
