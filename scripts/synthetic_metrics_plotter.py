...
class SyntheticMetricsPlotter:
    def __init__(
        ...
        met_order: str = None, 
        mod_order: str = None,
        sort_y_names: bool = False,
        show_proj_means: bool = False,
        show_ref_row: bool = False,
        show_alt_obs_rows: bool = False,
        ...
        self.results_dir = out_dir if out_dir is not None else "."
        
        # Test data (combined data if specified)
        ...
        self.reduced_set = bool(reduced_set)
        self.met_order = met_order 
        self.mod_order = mod_order
        self.sort_y_names = bool(sort_y_names)
        self.show_proj_means = bool(show_proj_means)
        self.show_ref_row = bool(show_ref_row) 
        self.show_alt_obs_rows = bool(show_alt_obs_rows)
        self.highlight_cmip = bool(highlight_cmip) 
    ...
    def _initialize_parameter(self):
        ...
        parsed_mip_names, parsed_test_names, parsed_case_ids = [], [], []
        if not self.test_combined: 
            # Strict 1:1 pairing (no broadcasting)
            mips, tests, caseids = self.pair_and_validate_lists({
                "test_mip": self.test_mip,
                "test_name": self.test_name,
                "test_id": self.test_case_id,
            })

            
            for raw_mip, raw_test, case_id in zip(mips, tests, caseids):
                parts = raw_mip.split(".")
                if len(parts) != 4:
                    raise ValueError(f"Invalid test format '{raw_mip}'. Expected 'a.b.c.d'")
                mip_id = f"{parts[0]}.{parts[1]}.{parts[2]}_{parts[3]}"
                parsed_mip_names.append(mip_id)
                parsed_test_names.append(raw_test)
    ...
    def pair_and_validate_lists(self, params: Dict) -> Tuple[List[str], List[str], List[str]]:
        """Coerce and validate 1:1 pairing; return tests, models, caseids only."""
        mips  = self._to_list(params.get("test_mip"))
        tests = self._to_list(params.get("test_name"))
        cids  = self._to_list(params.get("test_id"))

        if not (mips and tests and cids):
            raise ValueError(
                "test_mip, test_name, and test_id must all be provided and non-empty "
                f"(got lens: test_mip={len(mips)}, test_name={len(tests)}, test_id={len(cids)})."
            )
        n1, n2, n3 = len(mips), len(tests), len(cids)
        if not (n1 == n2 == n3):
            def preview(lst: List[str], k: int = 3):
                return lst[:k] + (["…"] if len(lst) > k else [])
            raise ValueError(
                "test_mip, test_name, and test_id must have the SAME length "
                f"(got {n1}, {n2}, {n3}).\n"
                f"  test_mip:  {preview(mips)}\n"
                f"  test_name: {preview(tests)}\n"
                f"  test_id:   {preview(cids)}"
            )
        return mips, tests, cids
    ...
    def _handle_mean_climate(self, metric: str) -> None:
        ...
        if self.test_combined:
            self.parameter.update(
                {"test_path": self.test_clim_dir, 
                 "test_name": self.test_clim_set,
                }
            )
        else:
            # sanity check (die earlier)---
    ...
    def _handle_variability_modes(self, metric: str) -> None:
        ...
        if self.test_combined:
            self.parameter.update(
                {"test_path": self.test_movs_dir, 
                 "test_name": self.test_movs_set,
                }
            )
        else:
            # sanity check ---
    ...
    def _handle_enso_metric(self, metric: str, debug: bool = False) -> None:
        ...
        if self.test_combined:
            self.parameter.update(
                {"test_path": self.test_enso_dir, 
                 "test_name": self.test_enso_set,
                }
            )
        else:
            # sanity check ---
    ...
    for mtype in metric_dict["type"]:
        if mtype == "portrait":
            ...
            if save_data:
                ...
                print("data_nor shape =", np.shape(data_nor))
                print("data_nor.T shape =", np.shape(data_nor.T))
                print("existing data_dict shape =", data_dict.shape)
                print("existing columns (n) =", len(data_dict.columns))
...
def portrait_metric_plot(
    ...
    ax.set_xticklabels(model_plot, rotation=45, va="bottom", ha="left", fontsize=fontsize * 0.95)
    ax.set_yticklabels(var_plot,  rotation=0,  va="center", ha="right", fontsize=fontsize * 0.95)
    ...
    if xlabel is not None:
        ax.set_xlabel(
            ...
            rotation=0,
            ha="right",
            labelpad=20,   # ← increase/decrease this
        )
    ...
    # center title over the data axes (exclude cbar/insets)
    # determine core title based on metric / region
    if "mean_climate" in (group, region):
        core = "Model Performance (Mean Climate)"
        use_region = True
    elif "variability_modes" in (group, region):
        core = "Model Performance (Variability Modes)"
        use_region = False
    elif "enso" in (group, region):
        core = "Model Performance (ENSO)"
        use_region = False
    else:
        core = "Model Performance"
        use_region = True
    
    # choose base title or core title
    main_title = base_title if base_title else core
    
    # construct suffix
    if use_region and region:
        suffix = f"{stat_name}, {region.upper()}"
    else:
        suffix = f"{stat_name}"
    
    # final title (single parentheses)
    if main_title.endswith(")"):
        title = main_title[:-1] + f", {suffix})"   # append inside existing (...)
    else:
        title = f"{main_title} ({suffix})"
    ...
    # draw once so bbox is known
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    
    # get suptitle height in figure coordinates
    bbox = st.get_window_extent(renderer=renderer)
    bbox = bbox.transformed(fig.transFigure.inverted())
    title_height = bbox.height
    
    # reserve exactly enough space for title (+ small pad)
    pad = 0.006   # tweak if needed
    top = 1.0 - title_height - pad
    top = max(0.5, min(0.98, top))  # safety clamp
...
def parcoord_metric_plot(
    ...
    xlabel = x_label if x_label is not None else None 
    ylabel = y_label if y_label is not None else f"{stat.upper()}" 
    ...
    if base_title is not None:        
        fig.suptitle(title, fontsize=fontsize * 1.2, fontweight="normal", x=0.01, ha="left")
    else:
        fig.suptitle(title, fontsize=fontsize * 1.2, fontweight="normal", x=0.5, ha="center")
